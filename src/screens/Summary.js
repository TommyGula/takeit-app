...
const Summary = ({ route, navigation }) => {
  ...
  useEffect(() => {
    ...
    const handleDeletedPlace = placeId => {
      showNotification(
        'Lo sentimos...',
        'El usuario canceló el encuentro.',
        null,
        'alert',
        () => {
          navigation.navigate('Home');
  ...
  if (loading) {
    return <Loading visible={loading}></Loading>;
  } else {
    return (
      <>
        {loading ? (
                ...
                {isConfirmed ? (
                  <>
                    <View style={summaryStyles.imageContainer}>
                      <Image
                        source={{
                          uri: 'https://cdn0.iconfinder.com/data/icons/social-messaging-ui-color-shapes-3/3/31-512.png',
                        }}
                ...
                <>
                  ...
                  <View
                    ...
                    <Button
                      color="secondary"
                      title="IR AL CHAT"
                      onPress={goToChat}></Button>
                    <Button
                      color="secondary"
                      title="IR AL PERFIL"
                      onPress={goToProfile}></Button>
                  </View>
                    ...
                    <MapView
                      ...
                      {region && summary ? (
                        <MapViewDirections
                          ...
                          apikey={
                            Config.GOOGLE_MAPS_PLATFORM_API_KEY ||
                            'AIzaSyCYMeIWU7pSQbh8C_Hc7ZMRXPqQyduVP8s'
                          }
                          strokeWidth={5}
                          onError={() =>
                            showNotification(
                              null,
                              'No fue posible trazar la ruta para esta dirección.',
                            )
                          }
                      ...
                      {summary && (
                        <Marker coordinate={summary} image={ParkIcon} />
                      )}
                    </MapView>
